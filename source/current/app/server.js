...
async function handleHomework(req,res){
  ...
  let targets=homeworkMathVerificationTargets(parsed).map(q=>({...q,bbox:bboxMap.get(String(q.sectionKey||'')+'|'+Number(q.page||0)+'|'+Number(q.printedNumber||0))||q.bbox||null}));let secondAi=null,secondCost=null,secondPassMs=0,render={mode:'high_res_page_png',requestedPages:[],renderedPages:[],failures:[],images:[]},crop={mode:'high_res_question_crop_png',crops:[],failures:[]};
  ...
  parsed.studentFileSource=studentFileSource;parsed.studentFileDriveUsed=!!studentFile.driveUsed;parsed.analysisArchitecture='two_pass_luna_question_crop_png';parsed.mathVerificationRenderMode=crop.mode;parsed.mathVerificationRequestedPages=render.requestedPages;parsed.mathVerificationRenderedPages=render.renderedPages;parsed.mathVerificationRenderFailureCount=render.failures.length+crop.failures.length;parsed.mathVerificationCropCount=crop.crops.length;parsed.mathVerificationCropFailureCount=crop.failures.length;parsed.firstPassMs=firstPassMs;parsed.secondPassMs=secondPassMs;parsed.totalAnalysisMs=totalMs;parsed.firstPassCostTry=Number(firstCost?.try||0);parsed.secondPassCostTry=Number(secondCost?.try||0);
