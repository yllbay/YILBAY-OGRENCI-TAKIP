...
function driveMetadataFromFile(file){const ap=file.appProperties||{};let course=String(ap.yilbayCourse||"").trim(),topic=String(ap.yilbayTopic||"").trim(),level=String(ap.yilbayLevel||"").trim(),title=String(ap.yilbayTitle||"").trim();if(!(course&&topic&&level)){const base=String(file.name||"").replace(/\.pdf$/i,"");const parts=base.split("__").map(x=>x.trim()).filter(Boolean);if(parts.length>=4){course=course||parts[0];topic=topic||parts[1];level=level||parts[2];title=title||parts.slice(3).join("__")}}course=normalizeCourseName(course);return {course,topic,level,title:title||String(file.name||"").replace(/\.pdf$/i,""),matched:!!(course&&topic&&level)}}
...
http.createServer(async(req,res)=>{
  try{
    ...
    if(u.pathname==="/health") return json(res,200,{ok:true,version:"0.9.2",integrations:integrationStatus()});
